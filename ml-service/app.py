...
CLASS_NAMES = ["Grade 0", "Grade 1", "Grade 2", "Grade 3", "Grade 4"]
TARGET_SIZE = 224
DB_PATH = os.environ.get("DB_PATH", "kneevision_cases.db")
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def retrieve_similar(query_vec, kl_grade=None, top_n=5):
    ...
    if kl_grade is not None:
        rows = conn.execute("""
            SELECT case_id, feature_vector, reference_image_path,
                   grade, grade_label,
                   previous_findings, suggested_actions,
                   progression_risk, lifestyle_factors,
                   recommended_followup, patient_profile
            FROM cases
            WHERE grade = ? AND clinical_meta_generated = 1
        """, (kl_grade,)).fetchall()
    else:
        rows = conn.execute("""
            SELECT case_id, feature_vector, reference_image_path,
                   ...
                   recommended_followup, patient_profile
            FROM cases
            WHERE clinical_meta_generated = 1
        """).fetchall()
    conn.close()
    ...
    for row in rows:
        ...
        scored.append({
            ...
            "klGrade": row[3],
            "gradeLabel": row[4],
            "previousFindings": row[5],
            "suggestedActions": row[6],
            "progressionRisk": row[7],
            "lifestyleFactors": row[8],
            "recommendedFollowup": row[9],
            "patientProfile": row[10],
        })
...
def quiz_question(difficulty: str = "medium"):
    ...
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(f"""
        SELECT case_id, reference_image_path, grade,
               previous_findings, suggested_actions,
               progression_risk, lifestyle_factors,
               recommended_followup, patient_profile
        FROM cases
        WHERE grade IN {grade_filter}
          AND clinical_meta_generated = 1
    ...
    return {
        "caseId": row[0],
        "imageBase64": image_base64,
        "previousFindings": row[3],
        "suggestedActions": row[4],
        "progressionRisk": row[5],
        "lifestyleFactors": row[6],
        "recommendedFollowup": row[7],
        "patientProfile": row[8],
    }
...
def quiz_answer(case_id: str):
    ...
    return {
        "correctGrade": row[0],
        "gradeLabel": row[1],
        "previousFindings": row[2],
        "suggestedActions": row[3],
        "progressionRisk": row[4],
        "lifestyleFactors": row[5],
        "recommendedFollowup": row[6],
        "patientProfile": row[7],
    }
# ─── End Quiz endpoint ────────────────────────────────────────────────────────